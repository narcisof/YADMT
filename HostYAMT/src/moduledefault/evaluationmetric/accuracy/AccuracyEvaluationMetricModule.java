 ...
 */
package moduledefault.evaluationmetric.accuracy;
 ...
 * @author evaristowb
 */
@ModuleAnnotation(name = "Acuidade")
@EvaluationMetricModuleAnnotation
public class AccuracyEvaluationMetricModule extends Observable implements EvaluationMetricModuleInterface {
    ...
    public void evaluation(ConfusionMatrix[] confusionMatrix) {
        ...
        jTextArea.append("Acuracia\n");
        for (int i = 0; i < mat.length; i++) {
            line_column = right = mat[i][i];
            for (int j = 0; j < mat[0].length; j++) {
                if (j != i) {
                    line_column += mat[i][j] + mat[j][i];
                }
            }
            if(line_column == 0)
                accuracy = 0;
