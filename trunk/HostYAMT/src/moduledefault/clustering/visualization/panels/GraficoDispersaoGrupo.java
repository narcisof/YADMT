...
import moduledefault.clustering.visualization.Classes.Aresta;
        ...
        if (atributo3 == 0) {
            if (atributo1 != 0 && atributo2 != 0) {
                ...
                if (TecnicasDispersao.getClusters() != null) {
                        ...
                        for (int l = 0; l < TecnicasDispersao.getClusters().size(); l++) {
                            if (TecnicasDispersao.getClusters().get(l).getNomeGrupo().equals(grupo)) {
                                for (int i = 0; i < TecnicasDispersao.getClusters().get(l).getGrupo().size(); i++) {
                                    ...
                                    Ponto p = new Ponto((padrao + 1) + "", x, y, 0, Color.black);
                                    ponto.add(p);
                    ...
                    TecnicasDispersao.setPontosGrupos(ponto);
                }
                if (TecnicasDispersao.isPintarPontoGrupos()) {
                    if (TecnicasDispersao.getPontosPintarGrupos() != null && TecnicasDispersao.getPontosPintarGrupos().size() != 0) {
                        for (int i = 0; i < TecnicasDispersao.getPontosPintarGrupos().size(); i++) {
                            g.setColor(TecnicasDispersao.getPontosPintarGrupos().get(i).getCor());
                            g.fillOval((int) TecnicasDispersao.getPontosPintarGrupos().get(i).getX(), (int) TecnicasDispersao.getPontosPintarGrupos().get(i).getY(), tamPixel + 1, tamPixel + 1);
                        }
                    }
                }
            }
                ...
                for (int j = 0; j < TecnicasDispersao.getVetorGrupos().size(); j++) {
                    ...
                    for (int l = 0; l < TecnicasDispersao.getClusters().size(); l++) {
                        if (TecnicasDispersao.getClusters().get(l).getNomeGrupo().equals(grupo)) {
                            for (int i = 0; i < TecnicasDispersao.getClusters().get(l).getGrupo().size(); i++) {
                                int padrao = TecnicasDispersao.getClusters().get(l).getGrupo().get(i).getNumero();
                                double x = TecnicasDispersao.getMatrizDados().getDataSet().get(padrao).getAtributos().get((atributo1 - 1)) * 100;
                                double y = TecnicasDispersao.getMatrizDados().getDataSet().get(padrao).getAtributos().get((atributo2 - 1)) * 100;
                                double z = TecnicasDispersao.getMatrizDados().getDataSet().get(padrao).getAtributos().get((atributo3 - 1)) * 100;
                                Color cor = null;
                                for (int k = 0; k < TecnicasDispersao.getMatrizDados().getClasses().size(); k++) {
                                    String classePadrao = TecnicasDispersao.getMatrizDados().getDataSet().get(padrao).getClasse();
                                    String classeMomento = TecnicasDispersao.getMatrizDados().getClasses().get(k);
